...
const SegmentReference = require('../media/segment_reference');
...
class SegmentIndex {
  ...
  mergeAndEvict(references, windowStart) {
    ...
    if (oldFirstRef) {
      ...
      assert(oldFirstRef.startTime == newFirstRef.startTime,
  ...
  updateEvery(interval, updateCallback) {
    assert(!this.timer_, 'SegmentIndex timer already started!');
    if (this.immutable_) {
      return;
    }
    if (this.timer_) {
  ...
  [Symbol.iterator]() {
    ...
    assert(iter != null, 'Iterator for 0 should never be null!');
    return iter;
  }
      ...
      for (let i = ref.partialReferences.length - 1; i >= 0; --i) {
        ...
        if ((time >= r.startTime) && (time < r.endTime)) {
          if (!allowNonIndepedent) {
            ...
            if (!r.isIndependent()) {
              logger.error('No independent partial segment found!');
              return null;
            }
          }
 ...
 * @export
 */
class MetaSegmentIndex extends SegmentIndex {
  /** */
  constructor() {
    super([]);
  ...
  fit(windowStart, windowEnd) {
    ...
    assert(false, 'fit() should not be used in MetaSegmentIndex!');
  }
...
module.exports = { SegmentIndex, MetaSegmentIndex };