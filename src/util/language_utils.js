...
static isoMap_ = new Map([
  ...
  static areLocaleCompatible(locale1, locale2) {
    ...
    // do it again.
    locale1 = this.normalize(locale1);
    locale2 = this.normalize(locale2);
  ...
  static areLanguageCompatible(locale1, locale2) {
    ...
    /** @type {!Array.<string>} */
    const locale1Components = this.disassembleLocale_(locale1);
    /** @type {!Array.<string>} */
    const locale2Components = this.disassembleLocale_(locale2);
  ...
  static isParentOf(possibleParent, possibleChild) {
    // Even through they SHOULD already be normalized, let's just be safe and
    // do it again.
    possibleParent = this.normalize(possibleParent);
    possibleChild = this.normalize(possibleChild);

    // Get all components. This should only be language and region
    // since we do not support dialect.
    /** @type {!Array.<string>} */
    const possibleParentComponents =
        this.disassembleLocale_(possibleParent);
    /** @type {!Array.<string>} */
    const possibleChildComponents =
        this.disassembleLocale_(possibleChild);
  ...
  static isSiblingOf(localeA, localeB) {
    // Even through they SHOULD already be normalized, let's just be safe and
    // do it again.
    localeA = this.normalize(localeA);
    localeB = this.normalize(localeB);

    // Get all components. This should only be language and region
    // since we do not support dialect.
    /** @type {!Array.<string>} */
    const localeAComponents = this.disassembleLocale_(localeA);
    /** @type {!Array.<string>} */
    const localeBComponents = this.disassembleLocale_(localeB);
  ...
  static normalize(locale) {
    ...
    language = this.isoMap_.get(language) || language;
  ...
  static areSiblings(a, b) {
    ...
    const baseA = this.getBase(a);
    const baseB = this.getBase(b);
  ...
  static relatedness(target, candidate) {
    ...
    if (this.isParentOf(candidate, target)) {
      return 3;
    }

    // Next is a sibling of the target language.
    if (this.isSiblingOf(candidate, target)) {
      return 2;
    }

    // Next is a child of the target language.
    if (this.isParentOf(target, candidate)) {
      return 1;
    }
  ...
  static getBase(lang) {
    ...
    major = this.isoMap_.get(major) || major;
  ...
  static getLocaleForVariant(variant) {
    ...
    if (variant.language) {
      return this.normalize(variant.language);
    }

    if (variant.audio && variant.audio.language) {
      return this.normalize(variant.audio.language);
    }

    if (variant.video && variant.video.language) {
      return this.normalize(variant.video.language);
    }
  ...
  static findClosestLocale(target, searchSpace) {
    ...
    /** @type {string} */
    const safeTarget = this.normalize(target);
    /** @type {!Set.<string>} */
    const safeSearchSpace = new Set();
    for (const option of searchSpace) {
      safeSearchSpace.add(this.normalize(option));
    }
    ...
    for (const option of safeSearchSpace) {
      if (this.isParentOf(option, safeTarget)) {
        return option;
      }
    }
    ...
    for (const option of safeSearchSpace) {
      if (this.isSiblingOf(option, safeTarget)) {
        return option;
      }
    }
    ...
    for (const option of safeSearchSpace) {
      if (this.isParentOf(safeTarget, option)) {
        return option;
      }
    }
