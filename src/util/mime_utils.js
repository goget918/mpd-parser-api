...
const TransmuxerEngine = require('../transmuxer/transmuxer_engine');
 ...
 */
  ...
  */
  ...
  static getFullType(mimeType, codecs) {
    ...
    if (codecs && !this.RAW_FORMATS.includes(mimeType)) {
  ...
  static getExtendedType(stream) {
    ...
    const extendedMimeParams = this.EXTENDED_MIME_PARAMETERS_;
    extendedMimeParams.forEach((mimeKey, streamKey) => {
      const value = stream[streamKey];
      if (streamKey == 'codecs' &&
        this.RAW_FORMATS.includes(stream.mimeType)) {
  ...
  static getNormalizedCodec(codecString) {
    const parts =
      this.getCodecParts_(codecString);
    const base = parts[0];
  ...
  static getCodecBase(codecString) {
    ...
    for (const codec of codecString.split(',')) {
      const parts = this.getCodecParts_(codec);
