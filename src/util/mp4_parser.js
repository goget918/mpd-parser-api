...
const {DataViewReader, Endianness} = require('./data_view_reader');
...
class Mp4Parser {
  ...
  /** */
  constructor() {
    /** @private {!Object.<number, this.BoxType_>} */
    this.headers_ = [];

    /** @private {!Object.<number, !this.CallbackType>} */
   ...
   *
   * @param {string} type
   * @param {!this.CallbackType} definition
   * @return {!shaka.util.Mp4Parser}
   * @export
   */
  box(type, definition) {
    const typeCode = this.typeFromString_(type);
    this.headers_[typeCode] = this.BoxType_.BASIC_BOX;
  ...
  fullBox(type, definition) {
    const typeCode = this.typeFromString_(type);
    this.headers_[typeCode] = this.BoxType_.FULL_BOX;
    this.boxDefinitions_[typeCode] = definition;
    return this;
  }
  ...
  parseNext(absStart, reader, partialOkay, stopOnPartial) {
    ...
    const name = this.typeToString(type);
    ...
    if (boxDefinition) {
      ...
      if (this.headers_[type] == this.BoxType_.FULL_BOX) {
  ...
  static children(box) {
    ...
    const headerSize = this.headerSize(box);
