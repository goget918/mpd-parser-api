...
const { SegmentIndex, MetaSegmentIndex } = require('../media/segment_index');
const LanguageUtils = require('./language_utils');
const ManifestParserUtils = require('./manifest_parser_utils');
const MimeUtils = require('./mime_utils');
 ...
 */
    ...
    */
    ...
    async combine_(
        ...
        for (const outputStream of outputStreams) {
            ...
            if (!ok) {
                ...
                // error.
                throw new Error("PERIOD_FLATTENING_FAILED");
            }
        ...
        for (const unusedStreams of unusedStreamsPerPeriod) {
            for (const stream of unusedStreams) {
                ...
                // an error.
                logger.error('Unused stream in period-flattening!',
    ...
    static async extendOutputSegmentIndex_(outputStream, firstNewPeriodIndex) {
        ...
        assert(streams, 'matched streams should be valid');
    ...
    static extendOutputStream_(
        ...
        // operation before.
        assert(outputStream.matchedStreams,
    ...
    static filterDuplicateCodecs_(codecs) {
        ...
        for (const codec of codecs) {
            ...
            if (!seen.has(shortCodec)) {
                ...
                logger.debug('Ignoring duplicate codec');
            }
        }
        return ret;
    }
    ...
    static concatenateStreams_(output, input) {
        ...
        //         Error.Code.INCONSISTENT_DRM_ACROSS_PERIODS);
        // }
        ...
        if (input.trickModeVideo) {
            if (!output.trickModeVideo) {
                ...
                output.trickModeVideo = PeriodCombiner.cloneStream_(
    ...
    areAVStreamsCompatible_(outputStream, candidate) {
        ...
        if (!PeriodCombiner.areAVStreamsExactMatch_(
    ...
    static isAudioStreamBetterMatch_(outputStream, best, candidate) {
        const LanguageUtils = LanguageUtils;
        const { BETTER, EQUAL, WORSE } = PeriodCombiner.BetterOrWorse;

        // An exact match is better than a non-exact match.
        const bestIsExact = PeriodCombiner.areAVStreamsExactMatch_(
            outputStream, best);
        const candidateIsExact = PeriodCombiner.areAVStreamsExactMatch_(
            outputStream, candidate);
        ...
        const channelsBetterOrWorse =
            PeriodCombiner.compareClosestPreferLower(
