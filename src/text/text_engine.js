...
const ClosedCaptionParser = require('../media/closed_caption_parser');
...
class TextEngine {
  ...
  static isTypeSupported(mimeType) {
    if (this.parserMap_[mimeType]) {
      // An actual parser is available.
      return true;
    }
    if (mimeType == MimeUtils.CEA608_CLOSED_CAPTION_MIMETYPE ||
  ...
  initParser(mimeType, sequenceMode, segmentRelativeVttTiming, manifestType) {
    ...
    const factory = this.parserMap_[mimeType];
    assert(
  ...
  async appendBuffer(buffer, startTime, endTime, uri) {
    ...
    const allCues = this.parser_.parseMedia(
      BufferUtils.toUint8(buffer), time, uri);
